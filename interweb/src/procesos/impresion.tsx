...
export default function Impresion() {
    ...
    const fetchData = () => {
        const apiUrl = `${API_BASE_URL}/procesos/pendientes-impresion`;
        setLoading(true);
    ...
    const handleSubmitSelected = () => {
        ...
        const endpoint = `${API_BASE_URL}/app/update-impresion`;
        axios.post(endpoint, { items: payload }, {
            headers: { 'Content-Type': 'application/json' }
        })
            .then(() => {
                alert('Elementos seleccionados para Impresion subidos correctamente!');
                setSelectedItems([]);
                fetchData();
            })
            .catch((error) => {
                console.error('Error al subir elementos para Impresion:', error);
                alert('Error al subir elementos para Impresion.');
            });
    };
    ...
    useEffect(() => {
        const fetchImpresion = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${API_BASE_URL}/app/impresion`);
                ...
                setError('Error al obtener datos de impresion');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '5px', backgroundColor: '#f9f9f9' }}>
                <h3 style={{ marginBottom: '15px', color: '#333' }}>Elementos Seleccionados - Impresión</h3>
